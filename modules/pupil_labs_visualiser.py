...
class PupilLabsVisualiser:
    def __init__(self, pupillabs_path):
        ...
        self.low_threshold = (
            2  # Used for cutting off values that are too far below trial mean
        )
        self.high_threshold = (
            3  # Used for cutting off values that are too far above trial mean
        )
        self.sg_w = (
            19  # Used in Savitzky-Golay filter (window size); reduce for less smoothing
        )
        self.s_omit = 0
        self.e_omit = 100000
    ...
    def filtering(self, eye_data):
        ...
        if self.choose_resample == "T":
            trial = pd.DataFrame(
                {
                    "newtime": np.arange(
                        eye_data["newtime"].min(), eye_data["newtime"].max(), 1 / 60
                    )
                }
            )
            interp_func = interp1d(
                eye_data["newtime"], eye_data["diameter_3d"], bounds_error=False
            )
        ...
        trial["xmm_vel"] = np.append(
            [np.nan],
            np.diff(trial["diameter_3d"])
            / np.append([np.nan], np.diff(trial["newtime"])),
        )
        ...
        trial["xmm_pp"] = np.where(
            np.abs(trial["xmm_vel"]) > out_bound, np.nan, trial["xmm_p"]
        )

        # Calculate lowest allowed value & remove values below this threshold
        low_bound = np.nanmean(trial["xmm_pp"]) - self.low_threshold * np.nanstd(
            trial["xmm_pp"]
        )
        trial["xmm_ppp"] = np.where(
            trial["xmm_pp"] < low_bound, np.nan, trial["xmm_pp"]
        )

        # Calculate highest allowed value & remove values above this threshold
        high_bound = np.nanmean(trial["xmm_pp"]) + self.high_threshold * np.nanstd(
            trial["xmm_pp"]
        )
        trial["xmm_ppp"] = np.where(
            trial["xmm_pp"] > high_bound, np.nan, trial["xmm_ppp"]
        )
    ...
    def main(self):
        ...
        pupils_list = pd.DataFrame(
            list(
                filter(
                    lambda x: "pupil_positions.csv" in x,
                    [
        ...
        test_file_name = pupils_list.files[
            1
        ]  # Choose a csv file from your list to process
        file_name = "JB-Imp1"
        ...
        player_output = pd.read_csv(
            f"{self.path_data}{test_file_name}", sep=","
        )  # Load csv file
        ...
        # Left eye (eye = 0)
        eye0_ts = (
            eye0.groupby("pupil_timestamp")["diameter_3d"].mean().reset_index()
        )  # Collapse over timestamp values
        eye0_ts["newtime"] = (
            eye0_ts["pupil_timestamp"] - eye0_ts["pupil_timestamp"].iloc[0]
        ).round(3)  # Create new timestamp starting at 0
        eye0_conf = eye0.groupby("pupil_timestamp")["confidence"].mean().reset_index()
        eye0_ts["confidence"] = eye0_conf["confidence"]

        # Right eye (eye = 1)
        eye1_ts = (
            eye1.groupby("pupil_timestamp")["diameter_3d"].mean().reset_index()
        )  # Collapse over timestamp values
        eye1_ts["newtime"] = (
            eye1_ts["pupil_timestamp"] - eye1_ts["pupil_timestamp"].iloc[0]
        ).round(3)  # Create new timestamp starting at 0
        ...
        eye0_true = eye0_ts[
            (eye0_ts["newtime"] > self.s_omit) & (eye0_ts["newtime"] < self.e_omit)
        ]
        eye1_true = eye1_ts[
            (eye1_ts["newtime"] > self.s_omit) & (eye1_ts["newtime"] < self.e_omit)
        ]
        ...
        plt.plot(
            eye0_true[eye0_true["confidence"] > 0.6]["newtime"],
            eye0_true[eye0_true["confidence"] > 0.6]["diameter_3d"],
            color="red",
        ...
        plt.plot(
            eye1_true[eye1_true["confidence"] > 0.6]["newtime"],
            eye1_true[eye1_true["confidence"] > 0.6]["diameter_3d"],
            color="red",
            label="High Confidence",
        )
        plt.title("Eye 1")
        ...
        filtered_eye0.to_csv(
            f"{self.path_filtered}{file_name}-Eye0.txt", index=False, sep="\t"
        )
        filtered_eye1.to_csv(
            f"{self.path_filtered}{file_name}-Eye1.txt", index=False, sep="\t"
        )
        ...
        plt.subplot(2, 1, 1)
        plt.plot(
            filtered_eye0["newtime"], filtered_eye0["diameter_3d"], label="Raw Data"
        )
        plt.plot(
            filtered_eye0["newtime"],
            filtered_eye0["xmm_int"],
            color="red",
            label="Filtered Data",
        )
        plt.title("Eye 0")
        ...
        plt.subplot(2, 1, 2)
        plt.plot(
            filtered_eye1["newtime"], filtered_eye1["diameter_3d"], label="Raw Data"
        )
        plt.plot(
            filtered_eye1["newtime"],
            filtered_eye1["xmm_int"],
            color="red",
            label="Filtered Data",
        )
        plt.title("Eye 1")
        ...
        sub_filtered_eye0 = filtered_eye0[
            (filtered_eye0["newtime"] >= start) & (filtered_eye0["newtime"] < end)
        ]
        plt.plot(
            sub_filtered_eye0["newtime"],
            sub_filtered_eye0["diameter_3d"],
            label="Raw Data",
        )
        plt.plot(
            sub_filtered_eye0["newtime"],
            sub_filtered_eye0["xmm_int"],
            color="red",
            label="Filtered Data",
        )
        plt.title("Eye 0")
