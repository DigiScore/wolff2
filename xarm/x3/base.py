        ...
        if self.REPORT_CONNECT_CHANGED_ID in self._report_callbacks.keys():
            for callback in self._report_callbacks[self.REPORT_CONNECT_CHANGED_ID]:
                self._run_callback(
                    callback,
                    {
                        "connected": self._stream and self._stream.connected
                        if main_connected is None
                        else main_connected,
                        "reported": self._stream_report
                        and self._stream_report.connected
            ...
            try:
                ...
                if self._keep_heart:
                    if (
                        ...
                        and self.arm_cmd.set_prot_flag(3) == 0
                    ):
                        prot_flag = 3
                    if (
                        prot_flag == 3
                ...
                if not self.reported:
                    ...
                    if not self.reported:
                        ...
                        if self.connected and (
                            connect_failed_cnt <= max_reconnect_cnts or prot_flag == 3
                        ):
                            time.sleep(2)
