...
class XArm(Gripper, Servo, Record, RobotIQ, BaseBoard, Track, FtSensor):
    ...
    def __get_tcp_motion_params(self, speed=None, mvacc=None, mvtime=None, **kwargs):
        ...
        spd = (
            ...
            if speed is None
            else min(max(float(speed), self._min_tcp_speed), self._max_tcp_speed)
        )
        acc = (
            self._last_tcp_acc
            if mvacc is None
            else min(max(float(mvacc), self._min_tcp_acc), self._max_tcp_acc)
        )
    ...
    def _set_servo_angle_relative(
        ...
        **kwargs,
        ...
        if self.version_is_ge(1, 8, 100):
            ...
            radius = radius if radius is not None else -1
            ret = self.arm_cmd.move_relative(
                joints, spd, acc, mvt, radius, True, False, only_check_type
            )
            ret[0] = self._check_code(ret[0], is_move_cmd=True)
            self.log_api_info(
    ...
    def shutdown_system(self, value=1):
        ...
        self.log_api_info(
            "API -> shutdown_system -> code={}".format(ret[0]), code=ret[0]
        )
        return ret[0]
    ...
    def set_joints_torque(self, joints_torque):
        ...
        self.log_api_info(
            "API -> set_joints_torque -> code={}, joints_torque={}".format(
                ret[0], joints_torque
            ),
            code=ret[0],
        )
    ...
    def get_firmware_config(self):
        ...
        code = (
            ...
            if cgpio_code == 0 and tau_code == 0
            else tau_code
        )
        return code, {
    ...
    def iden_joint_friction(self, sn=None):
        ...
        if sn[0] != ("L" if self.is_lite6 else "X") or sn[1] != axis_map.get(
            self.axis, ""
        ):
            self.log_api_info(
